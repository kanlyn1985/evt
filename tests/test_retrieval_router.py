...
def test_router_prefers_document_channel_for_standard_lookup() -> None:
    ...
    assert routed["query_type"] == "lifecycle_lookup"
    assert routed["channels"][0] == "document"
    assert any(hit["result_type"] == "document" and hit["doc_id"] == "DOC-000007" for hit in routed["hits"])
...
def test_router_uses_fact_channel_for_definition() -> None:
    ...
    assert routed["query_type"] == "definition"
    assert "facts" in routed["channels"]
    assert any(hit["result_type"] in {"fact", "wiki"} and hit["doc_id"] == "DOC-000006" for hit in routed["hits"])
